...
<div id="searchResponseAjax">
				...
				<c:when test="${transList != null && transList.size() > 0}">
					<c:forEach items="${transList}" var="each">
						<tr>
							...
							<td><a href="javascript:void(0);"><div class="btn btn-primary" onClick="loadTransaction('${each.value.pgPartnerTransId}');">REFUND</div></a></td>
							</c:when>
							<c:otherwise>
							<td></td>
							</c:otherwise>
	...
	jQuery(document).ready(function($) {
			...
			$('#refundTable').DataTable({"info":false,"searching":false,"order": [[ 5, "desc" ]]});
